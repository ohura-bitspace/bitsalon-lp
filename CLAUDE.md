...
サロン向け予約・顧客管理サービス **bitSalon** の紹介LP。トップページ、特定商取引法に基づく表記、プライバシーポリシー、利用規約の4ページ構成。
...
- **金額の参照元は `salon-reserve-mobile` リポジトリの `src/admin/features/settings/AdminPlanSettings.jsx`（`PLAN_GROUPS`）**。LP 側だけで金額を決めない。ただし**機能一覧は同期しない**。LP は訴求を絞っており、管理画面側がプロに挙げているホットペッパービューティー連携などを意図的に載せていない。
- **プラン別の機能差は `salon-reserve-mobile` の `src/features/index.js`（`PLAN_FEATURES`）が実態**。プランで実際に制限されているのはメッセージ・売上レポート（基本／詳細）・回数券の4つだけで、他の機能キーは定義だけで未使用。実装にない差をLPのプラン機能一覧に書かない。
- **LP の表示価格は税別、Stripe に登録した金額は税込**（スタンダード5,280円・プロ10,560円）。Stripe Tax は使わず、税込額をそのまま請求する方式。片方に合わせて他方を書き換えない。
...
- **LP に新規登録・申し込みの導線は置かない**。契約は「問い合わせ → 商談・見積もり → 申し込みフォームで規約に同意 → アカウント発行（社内作業） → 管理画面から課金開始」の流れで、初期費用が見積もり制かつ発行が手動のため、LP から直接申し込める状態にしない。申し込みフォーム（規約同意付き）は別に存在するが、商談後に個別で案内する。
